# GCN-Based Filtered Drag Modeling in Gas-Solid Fluidized Beds

Development of a Graph Convolutional Network (GCN) for sub-grid drag modeling in filtered Two-Fluid Model (fTFM) simulations of gas-solid fluidized beds, predicting filtered drift flux across dense bubbling to dilute pneumatic transport regimes within a single unified framework.
...
```bash
pip install torch torch-geometric numpy pandas matplotlib scikit-learn jupyter
```
